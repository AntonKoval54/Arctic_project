...
public class TaskManager {
    ...
    private void monitorTask(Task task) {
        String mpsOutput;
        try {
            mpsOutput = sshService.sendCommand("mps");
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        ...
        if (!runningJobs.contains(task.getJobId())) {
            if (!task.isComplete()) {
                ...
                System.out.println("Task retried: " + task.getCommand());
                scheduler.schedule(() -> monitorTask(task), task.getIntervalMinutes(), TimeUnit.MINUTES);
            } else {
                System.out.println("Task complete: " + task.getCommand());
                tasks.remove(task);
            }
        } else {
            // Задача еще выполняется, проверяем снова через небольшой интервал
            System.out.println("Task still running, checking again: " + task.getCommand());
